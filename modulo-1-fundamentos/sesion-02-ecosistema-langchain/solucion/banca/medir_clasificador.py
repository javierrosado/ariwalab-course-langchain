"""L2 · Banca — Banco Inti · medir_clasificador.py (checkpoint de referencia)

Corre las 26 consultas de recursos/golden/consultas-banca.json contra
clasificar_con_detalle(), e imprime acierto, matriz de confusión y reintentos.

Ejecutar desde la raíz del curso:
    python modulo-1-fundamentos/sesion-02-ecosistema-langchain/solucion/banca/medir_clasificador.py
"""
...
CONSULTAS_PROPIAS: list[tuple[str, str]] = [
    ("¿Cuánto puedo transferir al día desde mi cuenta?", "OTRO"),
    ("No reconozco un cobro de S/ 45 en mi tarjeta", "SOSPECHA_FRAUDE"),
    ("¿Qué tarjetas tengo asociadas a mi cuenta?", "CONSULTA_TARJETA"),
    ("Quiero ver mis operaciones de la semana pasada", "CONSULTA_MOVIMIENTOS"),
    ("¿Tengo saldo suficiente para pagar el alquiler?", "CONSULTA_SALDO"),
]
