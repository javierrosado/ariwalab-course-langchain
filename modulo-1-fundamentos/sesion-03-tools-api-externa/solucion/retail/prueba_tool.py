"""L3 · Retail — MercaSur · prueba_tool.py (checkpoint de referencia)

Ejecutar desde la raíz del curso:
    python modulo-1-fundamentos/sesion-03-tools-api-externa/solucion/retail/prueba_tool.py
...
# Identificador real de recursos/datasets/retail/pedidos.csv.
PEDIDO_REAL = "MS-2026-00001"
PEDIDO_INEXISTENTE = "MS-2026-99999"


def escenario_1_exito() -> None:
    print("  Escenario 1 · ÉXITO")
    print("  " + "-" * 68)
    resultado = track_order.invoke({"pedido_id": PEDIDO_REAL})
    print(f"  Entrada: pedido_id={PEDIDO_REAL}")
    print(f"  Salida:  {resultado}")
    assert "No existe" not in resultado, "Se esperaba un pedido real, no un mensaje de 'no existe'"
...
def escenario_2_dato_inexistente() -> None:
    ...
    print("  " + "-" * 68)
    resultado = track_order.invoke({"pedido_id": PEDIDO_INEXISTENTE})
    print(f"  Entrada: pedido_id={PEDIDO_INEXISTENTE}")
    print(f"  Salida:  {resultado}")
    assert "No existe" in resultado, "La tool debería decir explícitamente que no existe"
    print("  ✔ La tool dice 'no existe' — el agente no tiene margen para inventar un estado.\n")
...
def escenario_3_servicio_caido() -> None:
    ...
    plantilla = RUTAS[("retail", "pedidos.csv", "pedido_id")]
    ruta_con_fallo = plantilla.format(valor=PEDIDO_REAL) + "?_fallo=error503"
...
def main() -> None:
    print("=" * 72)
    print("  PRUEBA_TOOL · retail · track_order")
    print("=" * 72 + "\n")
    escenario_1_exito()
