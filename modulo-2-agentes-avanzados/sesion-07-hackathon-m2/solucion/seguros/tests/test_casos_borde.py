# -*- coding: utf-8 -*-
"""L7 · Seguros — Andina Seguros · tests/test_casos_borde.py (checkpoint de referencia)

Las 4 familias de caso borde, probadas con INVARIANTES y REPETICIÓN.

Ejecutar desde la raíz del curso:
    python modulo-2-agentes-avanzados/sesion-07-hackathon-m2/solucion/seguros/tests/test_casos_borde.py
...
def familia_1_entrada_ambigua(n: int) -> tuple[float, list[str]]:
    mensaje = "¿Mi SOAT está vigente? Choqué ayer y no sé si reportarlo. Placa VEX-958"
...
def familia_2_dato_ausente(n: int) -> tuple[float, list[str]]:
    def escenario(thread: str) -> str:
        responder("Hola, tengo una consulta sobre mi SOAT", thread_id=thread)
        responder("Es sobre la vigencia de mi póliza", thread_id=thread)
        responder("¿Hasta cuándo dura normalmente?", thread_id=thread)
        return responder("Mi placa es ZZZ-000, ¿está vigente?", thread_id=thread)
...
def familia_3_servicio_caido(n: int, chaos: bool) -> tuple[float, list[str]]:
    ...
    if chaos and cfg.DATA_SOURCE == "api":
        mensaje = "¿Mi SOAT está vigente? La placa es VEX-958"
        return tasa_exito(lambda thread: responder(mensaje, thread_id=thread), no_esta_vacia, n)
...
def main() -> int:
    p = argparse.ArgumentParser(description="Pruebas de estrés por invariantes — seguros")
    p.add_argument("--repeticiones", type=int, default=5)
    p.add_argument("--chaos", action="store_true")
    a = p.parse_args()

    print("\n" + "=" * 72)
    print("  PRUEBAS DE ESTRÉS POR INVARIANTES · seguros")
